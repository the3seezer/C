...
int solve(int, int, int, int b[][SIZE]);
...
int solve(int currentRow, int currentColumn, int number, int b[][SIZE]) {
	int moveNumber = 0;
	
	if (number == 64) {
		return -1;
	}
	if (b[currentRow][currentColumn] == 0) {
		while (moveNumber < 8) {
			if (nextRow(currentRow, moveNumber) != -1 && nextColumn(currentColumn, moveNumber) != -1) {
				b[currentRow][currentColumn] = number;
				if (solve(nextRow(currentRow, moveNumber), nextColumn(currentColumn, moveNumber), number + 1, b)) {
					return 1;
				}
			}
			++moveNumber;
		}
		b[currentRow][currentColumn] = 0;
	}
	return 0;	
}
