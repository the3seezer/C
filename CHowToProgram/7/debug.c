...
int exit( int, int, char[][ SIZE ], char[][ SIZE ] );
...
int main(void) {
    ...
    exit(currentRow, currentColumn, maze, copy );
...
int exit( int currentRow, int currentcolumn, char maze[][ SIZE ], char copyMaze[][ SIZE ] ) {
    copy( copyMaze, maze);
    copyMaze[ currentRow ][ currentcolumn ] = 'A'; // start 
} /* end function exit */
