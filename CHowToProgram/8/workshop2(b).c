...
int main( void ) {
    ...
    char recipe[320] = " 1 cup sour cream, 8 cakes, 1 cup milk, 1 teaspoon lemon juice, 1 cup sugar, 1 cup butter, 1 cup flour, 1 cup mayonnaise, 1 egg, 1 cup milk, 1/4 cup oil, white bread.";
    ...
    int knownFlag = 0;   // the known flag
    int unknownf1 = 0;  // unknown flag one
    int unknownf2 = 0;  // unknown flag two
    ...
    while( token != NULL ) { 
        ...
        // get the known flag if the token is known and vise versa
        if ( !mflag && strcmp( token, "," ) != 0 && strcmp( token, "." ) != 0 ) {
            for ( size_t i = 0; i < SIZE; i++ ) {
                for ( size_t j = 0; j < 2; j++ ) {
                    if ( strstr( token, ingredients[i][j] ) != NULL && *ingredients[i][j] != '\0' ) {
                        knownFlag = 1;
                        break;
                    }   // end if
                }   // end for
            ...
            // get the unkown flag if the token is unknown
            if ( !knownFlag ) { // token unknown 
                if( unknownf1 ) {
                    unknownf2 = 1;
                } // end
                unknownf1 = 1;
        ...
        if ( ( cFlag = strstr( token, "," ) ) != NULL || ( cFlag = strstr( token, "." ) ) != NULL ) {  // look for a comma, process the current ingredient
            ...
            else if ( unknownf1 || unknownf2 ) {  // if the unit is not descovered
                if ( ing_value )
                    printf( " %.3lf", ing_value );
                if ( *wordM != '\0' )  
