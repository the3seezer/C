/* 
    ...
    2/1/2024
    1:56 AM
    My string-Conversion Functions
*/
#include <stdio.h>
#include <math.h>
...
double strTod(const char * s, char ** endPtr ) {
    ...
    *endPtr = s; /*
    data * s == *string; and data ** endPtr == &( *sPtr );     NOTE: data; char, int, float, double, ...
    NOTE: 'endPtr = &s' accessing the given address of 's'(&). Derefferenced; '*endPtr' accessing the value of 'endPtr'; the address of 's' 
    NOTE: *endPtr = s Derefferenced '*endPtr' accessing the value of 'endPtr'; the address of sPtr. The value of 's'; the address of 'string'
    *NOTE: **endPtr = *s Derefferenced '**endPtr' accessing the value of '*endPtr'; the value of sPtr. Derefferenced '*s' accessing the value of 'string'  
    ...
    NOTE: endPtr = &s and ++( *endPtr ); derefferenced; will change the value of 'endPtr'; the address of 's'; in the current function
    NOTE: endPtr = &s and **( endPtr ) will change the address of 'endPtr'; in the current function
    NOTE; *endPtr = s and ++( *endPtr ); derefferenced; will change the value of 'endPtr'; the address of 'sPtr' and the value of s; the address of 'sPtr'; outside the function 
    NOTE: *endPtr = s and ++( endPtr ) will change the address of 'endPtr'
    *NOTE: **endPtr = *s ..., *Maybe will go direct to their values,i.e **endPtr[0] == endPtr[0], sPtr[0] and s[0], string[0]
                 */           
    ...
    while ( s[ i ] != '\0' ) {
        ...
        if ( !((int)s[ i ] >= 48 && (int)s[ i ] <= 57) && !num ) {
            // assign the pointer to a next location
            ++( *endPtr ); // NOTE: endPtr = &s and ++( *endPtr ) will change the address of pointer 's'. *endPtr = s and ++( endPtr ) Won't change any address
        } // end if

        // check for a negative sign
        if ( s[ i ] == '-' && (((int)s[i + 1]) >= 48 && ((int)s[i + 1]) <= 57) && !num ) {
            neg = 1;
        } // end if
